...
---
## QTimer
Работает на основе событийного цикла QT. Испускает сигнал timeout() через заданный интервал времени, после чего выполняется нужный код.
    connect(&timer_, &QTimer::timeout, this, &FileManager::checkFiles);
    timer_.setInterval(100);
}
выполнение метода checkFiles каждые 100мс.

## Описание  
**FileManager** – это консольное приложение для мониторинга изменений файлов.  
...
4. FileManager (Менеджер файлов).
   ...
   * Методы:
    ...
    * Singleton(instance): гарантирует, что у класса будет только один экземпляр, запрещает копировать объект.
---
